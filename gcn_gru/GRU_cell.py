...
class GRUCell_ori(RNNCell):
    ...
    def __init__(self, num_units, adj_mx, dropout, num_nodes, bias, input_size=None, reuse=None):
        ...
        :param reuse:
        """
        super(GRUCell_ori, self).__init__(_reuse=reuse)
        if input_size is not None:
            logging.warn("%s: The input_size parameter is deprecated.", self)
        self._num_nodes = num_nodes
        self._num_units = num_units
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or "dcgru_cell"):
            ...
            with tf.variable_scope("gates"):  # Reset gate
                # We start with bias of 1.0 to not reset and not update.
                r = Graph_matual(input_dim=input_shape[1],
                                 output_dim=self._num_units,
                                 adj=self.adj,
                                 act=tf.nn.sigmoid,
                                 dropout=self.dropout,
            ...
            with tf.variable_scope("gates"):  # Update gate
                u = Graph_matual(input_dim=input_shape[1],
                                 output_dim=self._num_units,
                                 adj=self.adj,
                                 act=tf.nn.sigmoid,
                                 dropout=self.dropout,
            ...
            with tf.variable_scope("candidate"):
                ...
                c = Graph_matual(input_dim=input_shape[1],
                                 output_dim=self._num_units,
                                 adj=self.adj,
                                 act=tf.nn.sigmoid,
                                 dropout=self.dropout,
